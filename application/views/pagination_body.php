        ...
        <div class="col-md-8 col-md-offset-2 bg-border">
            ...
            <table class="table table-striped table-hover text-left" width="100%">
                <thead>
                    ...
                    <th>#</th>
                    <th>學年度</th>
                    <th class="hidden-xs">活動日期</th>
                    <th class="hidden-xs">活動時間</th>
                    <th class="visible-xs-block">活動時間</th>
                    <th>活動地點</th>
                    <th>活動名稱</th>
                ...
                <tr>
                    <th><input type="checkbox" name="ac[]" value="<?php echo $booklist[$i]->activity_code; ?>"></th>
                    <td><?php echo ($page+$i+1); ?></td>
                    <td><?php echo $booklist[$i]->semester; ?></td>
                    <td class="hidden-xs"><?php echo $booklist[$i]->activity_date; ?></td>
                    <td class="hidden-xs"><?php echo date("H:i", strtotime($booklist[$i]->activity_time_start))."~".date("H:i", strtotime($booklist[$i]->activity_time_end)); ?></td>
                    <td class="visible-xs-block"><?php echo $booklist[$i]->activity_date." ".date("H:i", strtotime($booklist[$i]->activity_time_start))."~".date("H:i", strtotime($booklist[$i]->activity_time_end)); ?></td>
                    <td><?php echo $booklist[$i]->activity_location; ?></td>
                    ...
                    <td><a type="button" class="btn btn-warning btn-xs" href="<?php echo base_url("pagination/act_data_view?id=".$booklist[$i]->activity_code."&pre_p=".$previous_page)?>" >查看</a></td>
                </tr>
                <?php } ?>
                </tbody>
            </table>
